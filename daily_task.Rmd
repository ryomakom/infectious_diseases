...
# ---- 地方ベースの集計（案A） ----
# 全疾患×全地方について最新週の人口加重平均、前週比、季節Zスコア（過去全年・±2週）、
...
buildBlockSummary <- function(by_week, in_alert_now, in_attention_now, latest_yr_wk) {
  ...
  current_block <- block_weekly %>%
    filter(year_week == latest_yr_wk) %>%
    select(category, block, avg_value, n_prefs)

  previous_block <- block_weekly %>%
    filter(year_week == previous_yr_wk) %>%
    select(category, block, previous_avg_value = avg_value)

  # 季節ベースライン：過去年の同週±2週からmean/sdを算出
  hist <- block_weekly %>%
    filter(yr < latest_yr) %>%
