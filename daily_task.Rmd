...
# COVID流行期（2020〜2022）はベースラインから除外し、直近の非COVID年を3年分使う
latest_wk_int <- as.integer(latest_wk)
baseline_yrs_main <- head(setdiff((latest_yr - 1):(latest_yr - 7), COVID_EXCL_YRS), 3)
past_3_window_yr_wks <- sprintf("%04d-%02d",
  rep(baseline_yrs_main, each = 4),
  rep((latest_wk_int - 3):latest_wk_int, times = length(baseline_yrs_main))
)
...
# COVID流行期（外出自粛・マスク着用で多くの感染症が異常抑制）はベースラインから除外する
COVID_EXCL_YRS <- 2020:2022
    ...
    mutate(
      seasonal_mean = pmap_dbl(
        ...
        function(weekly_value_series, year_series, week_series) {
          ...
          hist_idx <- year_series < current_year & !year_series %in% COVID_EXCL_YRS & week_distance <= 2 & is.finite(weekly_value_series)
...
buildBlockSummary <- function(by_week, in_alert_now, in_attention_now, latest_yr_wk) {
  ...
  # COVID流行期（2020〜2022）はベースラインから除外し、直近の非COVID年を3年分使う
  baseline_yrs_block <- head(setdiff((latest_yr - 1L):(latest_yr - 7L), COVID_EXCL_YRS), 3L)
  past_3_window_yr_wks <- sprintf("%04d-%02d",
    rep(baseline_yrs_block, each = 4L),
    rep((latest_wk - 3L):latest_wk, times = length(baseline_yrs_block))
  )
  hist <- block_weekly %>%
    filter(year_week %in% past_3_window_yr_wks) %>%
    group_by(category, block) %>%
