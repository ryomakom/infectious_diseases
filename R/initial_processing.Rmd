...
# 保存フォルダ内のCSVファイルを列挙
csv_files <- list.files(path = "data", pattern = "-teiten\\.csv$", full.names = TRUE)
...
# 個別ファイルのパースエラーはスキップして続行する
diseases <- csv_files %>%
  lapply(function(f) {
    tryCatch(process_csv(f), error = function(e) {
      message("スキップ: ", basename(f), " — ", conditionMessage(e))
      NULL
    })
  }) %>%
...
# D3ビジュアライズ用のデータ
# regex で週テキストから日付・ラベルを抽出（文字位置に依存しないため堅牢）
...
cleaned_diseases %>% 
  write_excel_csv("data/merged/merged_data.csv")
